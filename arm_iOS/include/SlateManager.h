...
enum AutoBlockType : unsigned short int
{
    ...
    AUTO_LOC_QUALITY        = (1 << 11), /*!< Subscribe to localization quality */
...
enum SingleRequestBlockType : unsigned char
{
    ...
    REQ_DIRECTORY       = 10, /*!< ask for a file description block */
    REQ_NEW_PAGE        = 11  /*!< ask for new file content */
};
...
CLASS SlateManager
{
	...
	public :
         ...
         * \return true if the SlateManager is registred, false if not
         */
        bool registerListener(Listener *listener) ;

        /*!
         * \brief unregisterForEvent
         * \return true if the SlateManager is unregistred, false if not
         */
...
// BLE Special
    ...
    bool connect(NSString* peripheralName);
    bool startScan(BleScanListener *ble);
    bool stopScan();
    NSMutableArray* getDevicesList();
    CBPeripheral* getDeviceByID(int deviceID);
#endif
