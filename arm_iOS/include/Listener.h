...
public:
    ...
    * \param timecode :relative time code of the incoming event 
    */
    virtual void processEvent(Event &event, timecode_t timecode)=0;

    /*!
     * \brief connectionStatusChanged : Method invoked when connection status has changed
     * \param connected status true if connected false if not
     */
